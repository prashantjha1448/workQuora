                      ...
                      _SettingsTile(
                        ...
                        onTap: () async {
                          ...
                          final ok = await showDialog<bool>(
                            context: context,
                            builder: (_) => AlertDialog(
                              title: const Text('Set / Change Wallet PIN'),
                              content: Column(mainAxisSize: MainAxisSize.min, children: [
                                TextField(
                                    ...
                                    decoration: const InputDecoration(
                                        hintText: 'Old PIN (leave empty if first time)')),
                                TextField(
                                    controller: pinCtrl,
                                    obscureText: true,
                                    keyboardType: TextInputType.number,
                                    maxLength: 6,
                                    decoration: const InputDecoration(
                                        hintText: 'New PIN (4-6 digits)', counterText: '')),
                              ]),
                              actions: [
                                TextButton(
                                    onPressed: () => Navigator.pop(context, false),
                          ...
                          final failure = await ref
                              ...
                              .setWalletPin(
                                  ...
                                  oldPin: oldCtrl.text.trim().isEmpty
                                      ? null
                                      : oldCtrl.text.trim());
                          if (!context.mounted) return;
                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                              content: Text(failure?.message ?? 'Wallet PIN saved.'),
                              backgroundColor:
                                  failure == null ? AppColors.primary : AppColors.error));
                        },
                      ),
                      const Divider(height: 1, indent: 56),
                      _SettingsTile(
