...
import '../../../profile_kyc/application/profile_settings_controller.dart';
import '../../application/transactions_controller.dart';
import '../../data/models/wallet_transaction_model.dart';
import '../widgets/add_money_sheet.dart';
import '../widgets/settlement_breakdown.dart';
import '../widgets/add_bank_account_sheet.dart';

/// PIN-gate: balance is hidden until the worker verifies their wallet PIN.
final walletRevealedProvider = StateProvider<bool>((ref) => false);
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Consumer(builder: (context, ref, _) {
                              final revealed = ref.watch(walletRevealedProvider);
                              if (revealed) {
                                return Text(
                              ...
                              return GestureDetector(
                                onTap: () => _promptPin(context, ref),
                                child: Row(children: [
                                  Text('₹ ••••••',
                                      style: tt.displaySmall?.copyWith(
                                          color: Colors.white, fontWeight: FontWeight.w900)),
                                  const SizedBox(width: 10),
                                  const Icon(Icons.lock_open_rounded, color: Colors.white70, size: 20),
...
Future<void> _promptPin(BuildContext context, WidgetRef ref) async {
  ...
  final ok = await ref.read(profileSettingsControllerProvider).verifyWalletPin(pin);
  if (ok) {
    ref.read(walletRevealedProvider.notifier).state = true;
  } else if (context.mounted) {
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('Incorrect PIN. Set one in Settings if you haven\'t yet.'),
    ...
    return GestureDetector(
      onTap: tx.breakdown == null
          ...
          : () => showModalBottomSheet(
                ...
                builder: (_) => Padding(
                  padding: const EdgeInsets.all(16),
                  child: SettlementBreakdown(breakdown: tx.breakdown!),
                ),
              ),
      child: Container(
      margin: const EdgeInsets.only(bottom: 10),
